...
$('#zipSubmit').on('click', function(event) {
    ...
    $.ajax({
        url: "http://api.jambase.com/events?zipCode=" + zip + "&radius=25&page=0&api_key=uejmtjzcd7y25qhznzf7qkar&o=json",
        method: "GET",
        ...
        for (var i = 0; i < 9; i++) {
          //Get the events from the response
            ...
            venueZip = response.Events[i].Venue.ZipCode;
            ...
            $(".panel-body").append("<tr><td>" + moment(eventDate).format('LL') + "</td><td>" + artistNameArray + "</td><td>" + venueZip + "</td><td>" + "<button class='ticket'><a href=" + tickUrl + '>Buy Tickets</a></button></td>');
